...
func (c *PodVolumeRestoreReconciler) processRestore(ctx context.Context, req *velerov1api.PodVolumeRestore, pod *corev1api.Pod, log logrus.FieldLogger) error {
	...
	verify := podvolume.GetVolumesToVerifyIncludes(pod, req.Spec.Volume)
...
func (c *PodVolumeRestoreReconciler) processRestoreErrors(
	...
	req.Annotations[podvolume.PVRErrorsAnnotation] = strconv.FormatInt(int64(nErrors), 10)
	req.Annotations[podvolume.PVRVerifyErrorsAnnotation] = strconv.FormatInt(int64(nVerifyErrors), 10)
	req.Annotations[podvolume.PVRResticPodAnnotation] = os.Getenv("POD_NAME")
